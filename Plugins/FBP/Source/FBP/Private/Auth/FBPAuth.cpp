	...
	const FOnSignInFail& OnFail)
{
	...
	UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] FBPSignInWithEmailAndPassword"));

	firebase::auth::Auth* AuthRef = FBPGetAuth();

	UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] FBPSignInWithEmailAndPassword 0"));


	if (AuthRef == nullptr)
	{
		OnFail.ExecuteIfBound(TEXT("Firebase Auth is not initialized. Make sure Firebase App is properly set up."));
		return;
	}
	...
	if (UFBPFileUtilities::HasGoogleFile)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] FBPSignInWithEmailAndPassword 0.5 "));

		if (AuthRef == nullptr)
		{
			UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] AuthRef is null"));
		}

		UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] FBPSignInWithEmailAndPassword 0.7"));

		AuthRef->SignInWithEmailAndPassword(TCHAR_TO_UTF8(*Email), TCHAR_TO_UTF8(*Password)).Await(120 * 1000);


		UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] FBPSignInWithEmailAndPassword 1"));

		firebase::Future<firebase::auth::AuthResult> Result =
			AuthRef->SignInWithEmailAndPasswordLastResult();

		UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] FBPSignInWithEmailAndPassword 2"));
		...
		if (Result.status() == firebase::kFutureStatusComplete) {
			if (Result.error() == firebase::auth::kAuthErrorNone) {
				...
				FBPUser.LastSignedIn = User.metadata().last_sign_in_timestamp;
				FBPUser.IsAnonymous = User.is_anonymous();
				UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] FBPSignInWithEmailAndPassword Sucess"));

				OnSuccess.ExecuteIfBound(FBPUser);
			}
			else {

				UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] FBPSignInWithEmailAndPassword FAILED"));

				OnFail.ExecuteIfBound(FString(Result.error_message()));
			}
		}
#endif
		UE_LOG(LogTemp, Log, TEXT("[FBPGetAuth] END FBPSignInWithEmailAndPassword"));
	}
}
